...
* 40 MHz signal `gate_str_i` for gate circuit for strobe signal which comes out from CFD (Constant fraction discriminator)
* 20 MHz signal as a trigger for integrators, high state zeroes an integrator \
...
`en` signal is the output of the external D flip-flop which data input is `gate_str_o` and it's clock is `strb` which comes out from CFD (Constant fraction discriminator). `gate_str_o` is `clk40` delayed by 2 ns. `clk40` is the same as `gate_str_i`. The condition of strobe signal to be valid is that it has to has its risin edge when `gate_str_o` is high, then on stobe's falling edge (and after pipeline delay) data from integrator is displayed to the output. \
